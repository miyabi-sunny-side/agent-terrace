...
2. `skill` は宛先agentのruntimeに対応する `$HOME` 配下のskillディレクトリから
   検出した一覧でも検証する。
...
agent terrace は `GET /api/agents/{pane}/skills` で登録済みpaneのruntimeを確定し、
Claudeでは `~/.claude/skills`、Codexでは `~/.agents/skills` と
`~/.codex/skills` から `SKILL.md` を持つskillを列挙します。同じ一覧を送信時にも
検証し、agent-talkd側の検証と合わせて defense in depth を維持します。既定の
skill記法がないruntimeと、プロジェクト固有skillは現在の検出対象に含めません。
