...
共有環境の daemon / CLI 更新は今回の実装には含めていません。agent terrace
をデプロイする前に v0.4.0 以降へ更新します。
...
- skill と本文を別 field に保つ sticky composer
- `deliver` / `commit` の静的 skill allowlist
...
- skill 一覧は当面 `deliver`、`commit` の静的 allowlist とする。

運用環境で残っている確認事項:

- 共有 agent-talkd daemon / CLI を v0.4.0 以降へ更新
- Tailscale Serve 経由で Android Chrome の Screen / Letters を実機確認
- 空いている localhost port を既存 service と衝突しないよう決定

`5002` は候補の一つですが、デプロイ設定が確定するまでは予約済み port と
扱いません。
